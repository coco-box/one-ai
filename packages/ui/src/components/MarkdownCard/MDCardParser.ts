...
// 创建ast树节点
const genTreeNode = (node: Token & { vNodeKey?: string } | null): ASTNode => ({
...
export const tokensToAst = (tokens: Token[]): ASTNode[] => {
  ...
  tokens.forEach((tok: Token, idx: number) => {
    tok.vNodeKey = `one-markdown-content-key-${idx}`;
    tok.tokenIndex = idx;
...
const nodeToVNode = (node: Node): VNode | string | null => {
  ...
  // Vue2 需要通过 data.attrs 传递属性，避免例如 img 的 src 丢失
  return h(elementNode.tagName.toLowerCase(), { attrs: props }, children);
};
