...
import { consumeStream } from 'ai' // '../util/consume-stream';
import { SerialJobExecutor } from 'ai' // '../util/serial-job-executor';
import { type ChatTransport } from 'ai' // './chat-transport';
import { convertFileListToFileUIParts } from 'ai' // './convert-file-list-to-file-ui-parts';
import { DefaultChatTransport } from 'ai' // './default-chat-transport';
import {
...
import { defaultShouldStartNewMessage } from './default-should-start-new-message';
...
export abstract class AbstractChat<UI_MESSAGE extends UIMessage> {
  ...
  private shouldStartNewMessage?: ChatInit<UI_MESSAGE>['shouldStartNewMessage'];
  ...
  constructor({
    ...
    this.shouldStartNewMessage = shouldStartNewMessage ?? defaultShouldStartNewMessage;
  }
    ...
    try {
        ...
        this.jobExecutor.run(() =>
          job({
            ...
            write: () => {
              ...
              // 同步脏消息：并发智能体场景下 reasoning-delta/end 可能修改了旧消息，
              // 需先将它们同步到 UI 框架层（Vue/React），否则响应式不会触发更新。
              if (activeResponse.state.dirtyMessageIndices.size > 0) {
                for (const dirtyIdx of activeResponse.state.dirtyMessageIndices) {
                  const dirtyMsg = activeResponse.state.messages[dirtyIdx];
                  if (dirtyMsg) {
                    const uiIdx = this.state.messages.findIndex(
                      m => m.id === dirtyMsg.id,
                    );
                    if (uiIdx >= 0) {
                      this.state.replaceMessage(uiIdx, dirtyMsg);
                    }
                  }
                }
                activeResponse.state.dirtyMessageIndices.clear();
              }
      ...
      await consumeStream({
        stream: processUIMessageStream({
          ...
          shouldStartNewMessage: this.shouldStartNewMessage!,
        }),
        onError: error => {
          throw error;
        },
      ...
      this.onFinish?.({ message: activeResponse.state.message, messages: activeResponse.state.messages });
